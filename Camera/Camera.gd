...
func _ready() -> void:
	_noise.set_period(_NOISE_PERIOD)

func _physics_process(delta: float) -> void:
	if not _initialized:
		var player_origin := _player.translation
...
func _screen_shake_process(delta: float) -> void:
	...
	translation = _actual_position
	translation.x += _X_SHAKE_SCALE * shake * _noise.get_noise_2d(0, _shake_delta)
	translation.z += _Z_SHAKE_SCALE * shake * _noise.get_noise_2d(1, _shake_delta)
	rotation_degrees.y = _Y_ROTATION_BASE + _ROTATION_SHAKE_SCALE * shake * _noise.get_noise_2d(2, _shake_delta)
