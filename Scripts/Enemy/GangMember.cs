...
public class GangMember : Enemy
{
    ...
    private void Move(float delta)
    {
        ...
        if (attackDurationLeft == 0)
        {
            Velocity.x *= 0.9f;
            Velocity.z *= 0.9f;
            Vector3 direction = (FormationPosition - GlobalTransform.origin).Normalized();
            if (GlobalTransform.origin.DistanceTo(FormationPosition) > 0.5f)
            {
            ...
            {
                MoveAndSlide((FormationPosition - GlobalTransform.origin) / delta);
            }
